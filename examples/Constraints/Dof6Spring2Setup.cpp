    ...
    for (int i = 0; i < mb->getNumLinks(); ++i)
    {
        ...
        // radial axis
        {
			...
			if (angle > 1 * SIMD_PI / 180.f)
			{
				...
				torque = RadialTorqueDirs[i] * m_Ks[i] * (joint_lengths[i] * 0.5) * angle;

                // TODO disable damp ratio when debugging
    //            btScalar ratio = pow(fabs(angle / SIMD_PI), 1) * 10;
				//torque *= ratio;
